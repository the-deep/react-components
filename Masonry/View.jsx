 ...
 *    E) Function `state` returns either Redux or local component state
 *  - Infinite Scroll
 *
 *
 * How to use:
    ...
    <Masonry
      ...
      getItemHeight={Item.getItemHeight}
...
export default class Masonry extends React.PureComponent {
    static propTypes = {
        ...
        getItemHeight: PropTypes.func.isRequired, // eslint-disable-line react/no-unused-prop-types
        getItemColumnSpan: PropTypes.func, // eslint-disable-line react/no-unused-prop-types
    }

    static defaultProps = {
        alignCenter: true,
        containerClassName: 'masonry collection-group',
        isItemsChanged: (a, b) => a.length !== b.length,
        layoutClassName: 'masonry-view',
        pageClassName: 'masonry-page',
        loadingElement: (
        ...
        viewMode: 'default',
        state: {},
        getItemColumnSpan: defaultColumnSpanSelector,
    }
    ...
    componentWillReceiveProps(nextProps) {
        if (nextProps.isItemsChanged(nextProps.items, this.props.items) ||
    ...
    computeLayout(props, rearrange = false) {
        ...
        const pages = itemsToLayout.reduce((workingPages, itemProps) => {
            ...
            const columnSpan = Math.min(maxColumns, getItemColumnSpan(props.state, itemProps));
